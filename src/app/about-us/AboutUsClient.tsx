...
import { useAboutCms } from "@/hooks/about/useAboutCms";
import type { AboutCmsData, CountDownItem } from "@/types/models";

interface AboutUsClientProps {
  initialData?: AboutCmsData | null;
}

export default function AboutUsClient({ initialData = null }: AboutUsClientProps) {
  const { bannerFields, overviewFields, mvFields, valuesFields } = useAboutCms(initialData);

  const countdownData = useMemo<CountDownItem[]>(() => {
    if (overviewFields?.statistics && overviewFields.statistics.length > 0) {
      return overviewFields.statistics
        .slice()
    ...
    return [];
  }, [overviewFields?.statistics]);

  const valueCards = useMemo(() => {
    if (!valuesFields) return [];

    const cards = [];

    if (valuesFields.integrity_title || valuesFields.integrity_description) {
      cards.push({
        title: valuesFields.integrity_title,
        desc: valuesFields.integrity_description,
        icon: (
    ...
    if (valuesFields.customer_excellence_title || valuesFields.customer_excellence_description) {
      cards.push({
        title: valuesFields.customer_excellence_title,
        desc: valuesFields.customer_excellence_description,
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path
    ...
    if (valuesFields.performance_collaboration_title || valuesFields.performance_collaboration_description) {
      cards.push({
        title: valuesFields.performance_collaboration_title,
        desc: valuesFields.performance_collaboration_description,
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path
    ...
    if (valuesFields.community_impact_title || valuesFields.community_impact_description) {
      cards.push({
        title: valuesFields.community_impact_title,
        desc: valuesFields.community_impact_description,
        icon: (
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
  ...
  return (
    <>
      {bannerFields && bannerFields.banner_image && (
        <PageBanner
          image={bannerFields.banner_image}
          title={bannerFields.title || bannerFields.image_alt || "About Us"}
        />
      )}

      {/* Company Overview Section */}
      {overviewFields && (
        <section id="company-overview" className="pt-[80px] lg:pt-[120px] bg-white">
          <div className="container mx-auto">
            <RevealText>
              {overviewFields.head_title && <SubTitle>{overviewFields.head_title}</SubTitle>}
              {overviewFields.title && (
            ...
            <div className="flex flex-col lg:flex-row gap-[40px] lg:gap-[80px] mb-[50px]">
                ...
                <div className="lg:w-[580px] w-full h-[500px] lg:h-auto lg:shrink-0 flex flex-col">
                  <RevealImage className="w-full h-full flex-1">
                    <div className="w-full h-full relative rounded-[10px] overflow-hidden">
                      <Image
                        src={overviewFields.image}
                        alt={overviewFields.image_alt || "Company Overview"}
                        fill
              ...
              <div className="w-full flex-1 flex flex-col gap-[24px]">
                {overviewFields.sub_title && (
                  <RevealText delay={0.1}>
                    <div
                      className="text-[#1E1E1E] text-[2rem] font-bold leading-tight [&_p]:m-0"
                      dangerouslySetInnerHTML={{ __html: overviewFields.sub_title }}
                    />
                  </RevealText>
                )}

                {overviewFields.description && (
                  <RevealText delay={0.2}>
                    <div
                      className="text-[#666] text-[1rem] font-normal leading-relaxed [&_strong]:text-[#1E1E1E] [&_strong]:font-bold [&_p]:mb-4 [&_p:last-child]:mb-0"
                      dangerouslySetInnerHTML={{ __html: overviewFields.description }}
                    />
                  </RevealText>
                )}
              </div>
      ...
      {countdownData.length > 0 && (
        <section id="history-legacy" className="lg:py-[50px] pb-[50px]">
          <div className="container mx-auto">
      ...
      {mvFields && (
        <section id="vision-mission" className="bg-[#F9F9F9] py-[80px] lg:py-[100px]">
          <div className="container mx-auto">
            <div className="pb-[50px] mb-[50px] border-b-[4px] border-[#E5E5E5]/80">
              <div className="flex flex-col lg:flex-row gap-[40px] lg:gap-[80px]">
                <div className="w-full lg:w-1/2 lg:py-[24px] flex flex-col gap-[20px] lg:gap-[60px] justify-center">
                  {(mvFields.mission_title || mvFields.mission_description) && (
                    <RevealText delay={0.1}>
                      <div>
                        {mvFields.mission_title && (
                          <h3 className="text-[#1E1E1E] text-[2.5rem] font-bold mb-[16px]">
                            {mvFields.mission_title}
                          </h3>
                        )}
                        {mvFields.mission_description && (
                          <div
                            className="text-[#666] text-[1rem] font-normal text-justify leading-relaxed [&_p]:m-0"
                            dangerouslySetInnerHTML={{ __html: mvFields.mission_description }}
                          />
                        )}
                      </div>
                    </RevealText>
                  ...
                  {(mvFields.vision_title || mvFields.vision_description) && (
                    <RevealText delay={0.2}>
                      <div>
                        {mvFields.vision_title && (
                          <h3 className="text-[#1E1E1E] text-[2.5rem] font-bold mb-[16px]">
                            {mvFields.vision_title}
                          </h3>
                        )}
                        {mvFields.vision_description && (
                          <div
                            className="text-[#666] text-[1rem] font-normal text-justify leading-relaxed [&_p]:m-0"
                            dangerouslySetInnerHTML={{ __html: mvFields.vision_description }}
                          />
                        )}
                      </div>
                    </RevealText>
                  ...
                  <div className="w-full lg:w-1/2">
                    <RevealImage>
                      <div className="w-full h-[300px] lg:h-[450px] relative rounded-[5px] overflow-hidden">
                        <Image
                          src={mvFields.image}
                          alt={mvFields.image_alt || "Our Mission & Vision"}
                          fill
                          className="object-cover"
            ...
            {valuesFields && (
              <div>
                <RevealText delay={0.1}>
                  {valuesFields.head_title && <SubTitle>{valuesFields.head_title}</SubTitle>}
                  {valuesFields.title && (
                  ...
                  {valuesFields.description && (
                    <div
                      className="text-[#5E5E5E] text-[1rem] font-normal mb-[3rem] [&_p]:m-0"
                      dangerouslySetInnerHTML={{ __html: valuesFields.description }}
                    />
                  )}
                </RevealText>
                ...
                {valueCards.length > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-[24px]">
                    {valueCards.map((item, index) => (
                      <RevealText key={item.title || index} delay={0.1 * index}>
                        <div className="p-[1rem] rounded-[12px] border border-[#F2F2F2] bg-[#FDFDFD] h-full">
                          ...
                          {item.desc && (
                            <div
                              className="text-[#666] text-[0.8rem] font-normal leading-relaxed [&_p]:m-0"
