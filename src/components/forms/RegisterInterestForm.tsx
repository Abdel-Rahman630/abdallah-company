...
export default function RegisterInterestForm({ eventId }: RegisterInterestFormProps) {
  ...
  const onSubmit = async (data: RegisterInterestFormData) => {
    ...
    try {
      ...
      setStatus("error");
      const errorMessage = (err as Error)?.message || t("eventForm.errorDesc");
      ...
      {status === "error" && (
        <div className="mb-[24px] p-[16px] rounded-[8px] bg-red-50 border border-red-200">
          <p className="text-red-800 font-bold text-[0.9rem]">
            {t("eventForm.errorTitle")}
          </p>
          <p className="text-red-700 text-[0.85rem]">
            {errorMessage || t("eventForm.errorDesc")}
          </p>
        </div>
      )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-[1rem]" noValidate>
          <div className="flex flex-col gap-[4px]">
            <FormInput
              ...
              error={!!errors.name}
              {...register("name", { required: t("eventForm.nameRequired") })}
            />
          ...
          <div className="flex flex-col gap-[4px]">
            <FormInput
              ...
              error={!!errors.phone}
              {...register("phone", { required: t("eventForm.phoneRequired") })}
            />
          ...
          <div className="flex flex-col gap-[4px]">
            <FormInput
              ...
              error={!!errors.email}
              {...register("email", { required: t("eventForm.emailRequired") })}
            />
          ...
          >
            {status === "loading" ? t("eventForm.sending") : t("eventForm.send")}
            <svg className="shrink-0 rtl:rotate-180 transition-transform" xmlns="http://www.w3.org/2000/svg" width="13" height="10" viewBox="0 0 13 10" fill="none">
